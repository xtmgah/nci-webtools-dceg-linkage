...
class QueueProcessor(SubscriptionListener):
    ...
    def queueConsumer(self, client, frame):
        ...
        try:
            ...
            resp = jsonify({ 
                "message": "The batch process has begun. The results will be emailed to '" + recipient + "'"
                })
        except Exception, e:
            print "Error Type: {0} Error: {1} Trace: {2}".format(errorType, error, traceback)
            resp = jsonify({"message": e.args})
    ...
    def startQueue(self):
        try:
            ...
            if self.QUEUE is not None:
                ...
                headers = {
                    # client-individual mode is necessary for
                    # concurrent processing (requires ActiveMQ >= 5.2)
                    StompSpec.ACK_HEADER: StompSpec.ACK_CLIENT_INDIVIDUAL,
                    # the maximal number of messages the broker will let 
                    # you work on at the same time
                    'activemq.prefetchSize': self.PREFETCH
                }

                client.subscribe(self.Q_NAME, headers, listener=self)
        except Exception, e:
            errorType, error, traceback = sys.exc_info()
            print "Error Type: {0} Error: {1} \n {2}".format(errorType, error, traceback)
    ...
    def __init__(self, CONFIG_FILE=None, Q_NAME=None, Q_URL=None, Q_PORT=None, Q_ERROR=None, PRODUCT_NAME="", MAIL_HOST=None, MAIL_ADMIN=None, EMAIL_BODY=None, PREFETCH=100):
        if CONFIG_FILE is not None:
            ...
            self.QUEUE = StompConfig(self.queueConfig[Q_URL])
            ...
            self.Q_PORT = int(self.QUEUE_CONFIG[Q_PORT])

            print self
            super(QueueProcessor, self).__init__(self.queueConsumer)

            self.QUEUE = StompConfig(uri="failover:({0})?startupMaxReconnectAttempts=-1,initialReconnectDelay=300000".format(self.queueConfig[Q_URL]))
            self.QUEUE_CONFIG = PropertyUtil(self.CONFIG_FILE)
            self.rscript = robjects.r('"source(scriptName.R)"')
